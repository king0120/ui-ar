...
const fields = [{
    ...
    title: 'Age',
    value: ['Child', 'Teen', 'Teens-20', '20s-30s', '30s-40s', '40s-50s', '50s-60s', '60s-70s', 'Over 70'],
    category: 'ageRange',
    searchOnly: false
}, {
    title: 'Ethnicity',
    ...
    title: 'Eye Color',
    value: ['Brown', 'Hazel', 'Blue', 'Green', 'Gray', 'Amber', 'Other', 'Unknown'],
    category: 'eyeColor',
    searchOnly: true
}, {
    title: 'Hair Color',
...
const FormRender: FC<any> = (props) => (
    <form onSubmit={props.handleSubmit}>
        ...
        {fields.filter((f) => props.search || !f.searchOnly).map(field => (
            <div className='spec-container' key={field.title}>
                ...
                <div className='spec-checkboxes flex flex-wrap'>
                    {field.value.map(option => (
                        <SmartCheckbox key={option} option={option}
                                       category={field.category}/>)
                    )}
                </div>
            </div>
        ))}
        {props.button ? <Button variant='contained' color="primary" className={'float-right m-1'}
                                type='submit'>Submit</Button> : null}
    </form>
);

export const TalentSpecificationsForm: FC<any> = ({search, button, breakdown = {}, onSubmit = () => {}}) => {
