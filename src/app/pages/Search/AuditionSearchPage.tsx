...
import {Button, createStyles, Divider, InputBase, makeStyles, Paper, Theme, Typography} from '@material-ui/core';
...
const AuditionSearchResult = (props: {results: any, history: any}) => {
    ...
    const sliceToShow = props.results && props.results.slice(selected * 10, (selected * 10) + 10);
    return (<>
        <div className="w-full min-w-full table-responsive">
            ...
            >
                {sliceToShow.map((result: any) => (
                    <>
                        <div className='w-full flex content-between' key={result.id}
                             onClick={() => props.history.push(`/audition/${result.id}`)}>
                            <div
                                className="pl-64 flex flex-col justify-center align-center truncate font-600">
                                <Typography
                                    variant="h6">{result.project.name} at {result.project.organization.name}</Typography>
                                <Typography variant="body2">Audition
                                    Name: {result.name}</Typography>
        ...
        <Container>
            ...
            <Paper>
                <div className="flex items-center justify-between px-16 h-64">
                    ...
                    <Typography
                        className="text-11 font-500 rounded-4 text-white bg-blue px-8 py-4">{results ? results.length : 0}</Typography>
                </div>
                {!loading && results && <AuditionSearchResult results={results} history={props.history}/>}
            </Paper>
        </Container>
    );
};
